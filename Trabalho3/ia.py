...
class ProblemaBuscaSimulatedAnnealing(Problema):
	"""
	...
	def __init__(self, estado_inicial, operadores, qt_enfer, qt_turnos, melhor_avaliado, avaliar_estado, metodo, temperatura=350):
	   super().__init__(estado_inicial, operadores, qt_enfer, qt_turnos)
	   self.melhor_avaliado = melhor_avaliado
	   self.avaliar_estado = avaliar_estado
	   self.metodo = metodo
	...
	while True:
		...
		if not no.estado in visitados:
			...
			if(problema.metodo == 1):
				# Se o tipo de busca for tempera simulada
				if (best != []):
					if(compare(melhor_no, best[0]) == False):
						melhor_no = best[0]
						# Insere o aleatorio na fila
						ind = randint(0, len(gerados)-1)
						nos = insert_no(gerados[ind], nos)
